...
public class RevPolishCalc implements Calculator {
  ...
  //private OpStack operations; -used for opstack push.
  //used array instead as could not get it to work
  ...
  public float evaluate(String what) throws InvalidExpressionException {
    float output = 0;
    if (what == null || what.equals("")) { //when inputted nothing.
    ...
    try {
      if (scan.hasNext()) {
        do {
          if (scan.hasNextFloat()) {
            ...
            //implentation with OpStack could not get to work
            //          for (int i = 0; i < Symbol.values().length; i++) {
            //            operations.push(Symbol.values()[i]);
            //            if (Symbol.values()[i].equals(token)) {    
            //              opp = Symbol.values()[i];
            //              break;
            //            }
            //          }
            ...
            if (opp == Symbol.DIVIDE) {
              ...
              throw new InvalidExpressionException("Invalid operation token");
            }
          }
      ...
      if (!values.isEmpty()) {
        throw new InvalidExpressionException("Expression unbalanced");
      }
